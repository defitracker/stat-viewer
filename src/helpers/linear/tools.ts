 ...
 */
function isArrayOrMatrix(designMatrix: any) {
 ...
 */
function addConstant(designMatrix: Matrix) {
 ...
 */
function isConstantPresent(designMatrix: Matrix) {
 ...
 */
function lagMatrix(x: Array<number>, nLags: number, logging: boolean) {
  if (!isInteger(nLags) || isNegative(nLags)) {
    throw new Error(
      "nlags needs to be a positive integer to calculate the lagMatrix."
    );
  }
